func! searchlabels#search#new() abort
  let s = {}
  ...
  func! s.init(input, repeatmotion, reverse) abort
    ...
    let self.prefix = searchlabels#search#get_cs(a:input, g:searchlabels#opt.use_ic_scs).'\V'
    ...
    " if !a:repeatmotion && !searchlabels#is_sneaking() | let self._search_options .= 's' | endif
  endf
  ...
  func! s.get_onscreen_searchpattern(w) abort
    ...
    let wincol_rhs  = 2 + (winwidth(0) - searchlabels#util#wincol1()) + wincol_lhs
  ...
  func! s.hasmatches(n) abort
    ...
    while 1
      ...
      if 0 == matchpos[0] "no more matches
        break
      elseif 0 != searchlabels#util#skipfold(matchpos[0], self._reverse)
        continue
      endif
...
func! searchlabels#search#get_cs(input, use_ic_scs) abort
  if !a:use_ic_scs || !&ignorecase || (&smartcase && searchlabels#util#has_upper(a:input))
    return '\C'
  endif
  return '\c'
endf
...
let g:searchlabels#search#instance = searchlabels#search#new()
